...
def test_tab_name_owner_gets_plain_name():
    assert _client._tab_name("Transactions", 111) == "Transactions"
    assert _client._tab_name("Budgets", 111) == "Budgets"


def test_tab_name_other_user_gets_suffixed_name():
    assert _client._tab_name("Transactions", 222) == "Transactions_222"
    assert _client._tab_name("Transactions", 999) == "Transactions_999"


def test_tab_name_no_allowed_ids_configured(monkeypatch):
    monkeypatch.setattr(_client, "ALLOWED_IDS", [])
    # With no configured owner, nobody gets the plain, un-suffixed tab.
    assert _client._tab_name("Transactions", 111) == "Transactions_111"


def test_tab_name_different_users_get_different_tabs():
    assert _client._tab_name("Transactions", 222) != _client._tab_name("Transactions", 333)
...
def test_is_transient_error_true_for_known_transient_statuses(status):
    assert _client._is_transient_error(_http_error(status)) is True


@pytest.mark.parametrize("status", [400, 401, 403, 404])
def test_is_transient_error_false_for_client_errors(status):
    assert _client._is_transient_error(_http_error(status)) is False


def test_is_transient_error_true_for_connection_error():
    assert _client._is_transient_error(ConnectionError("network blip")) is True


def test_is_transient_error_true_for_timeout_error():
    assert _client._is_transient_error(TimeoutError("timed out")) is True


def test_is_transient_error_false_for_generic_exception():
    assert _client._is_transient_error(ValueError("not a transient issue")) is False


# --- _retry_call ---

def test_retry_call_returns_result_on_first_success():
    assert _client._retry_call(lambda: "ok") == "ok"
...
def test_retry_call_succeeds_after_transient_failures():
    ...
    assert _client._retry_call(flaky) == "recovered"
    assert calls["n"] == 3
...
def test_retry_call_raises_immediately_on_non_transient_error():
    ...
    with pytest.raises(HttpError):
        _client._retry_call(bad_request)
...
def test_retry_call_raises_after_exhausting_retries():
    ...
    with pytest.raises(HttpError):
        _client._retry_call(always_flaky)
    assert calls["n"] == _client._MAX_RETRIES


# --- _get_sheets_service caching ---

def test_get_sheets_service_is_cached(monkeypatch):
    monkeypatch.setattr(_client, "_service_cache", None)
    monkeypatch.setattr(_client.os.path, "exists", lambda path: True)
    ...
    monkeypatch.setattr(_client, "build", fake_build)
    monkeypatch.setattr(_client.Credentials, "from_service_account_file", lambda *a, **k: Mock())

    first = _client._get_sheets_service()
    second = _client._get_sheets_service()
...
def test_get_sheets_service_raises_if_credentials_missing(monkeypatch):
    monkeypatch.setattr(_client, "_service_cache", None)
    monkeypatch.setattr(_client.os.path, "exists", lambda path: False)

    with pytest.raises(FileNotFoundError):
        _client._get_sheets_service()
...
def mock_sheets_service(monkeypatch):
    ...
    monkeypatch.setattr(_client, "_service_cache", mock_service)
    ...
    # in the next test's assertions. Mutate the shared set object in
    # place (rather than replacing it via monkeypatch) since
    # transactions.py and budgets.py imported this exact object by
    # reference — swapping in a new set here would leave their copies
    # pointing at the old, un-cleared one.
...
async def test_get_recent_transactions_with_index_maps_rows_correctly(mock_sheets_service):
    ...
    page, has_more = await storage.get_recent_transactions_with_index(111, n=10)
...
async def test_get_recent_transactions_with_index_respects_n(mock_sheets_service):
    ...
    page, has_more = await storage.get_recent_transactions_with_index(111, n=2)
    assert len(page) == 2
...
async def test_get_recent_transactions_with_index_pagination_offset(mock_sheets_service):
    ...
    page, has_more = await storage.get_recent_transactions_with_index(111, n=2, offset=2)
...
async def test_get_recent_transactions_with_index_last_page_has_no_more(mock_sheets_service):
    ...
    page, has_more = await storage.get_recent_transactions_with_index(111, n=2, offset=4)
...
async def test_get_all_transactions_with_index_maps_rows_correctly(mock_sheets_service):
    ...
    result = await storage.get_all_transactions_with_index(111)
    assert result == [
        (1, ["01.08.2026", "Expense", "Кафе", 100, "A"]),
        (2, ["02.08.2026", "Expense", "Таксі", 50, "B"]),
    ]
...
async def test_update_transaction_row_uses_correct_a1_range(mock_sheets_service):
    ...
    await storage.update_transaction_row(111, 2, "02.08.2026", "Expense", "Таксі", 75, "B updated")
...
async def test_delete_transaction_row_targets_correct_index(mock_sheets_service):
    ...
    result = await storage.delete_transaction_row(111, 2)
...
async def test_append_transactions_batch_makes_a_single_api_call(mock_sheets_service):
    ...
    await storage.append_transactions_batch(111, entries)
...
async def test_tab_exists_metadata_checked_once_across_multiple_appends(mock_sheets_service):
    ...
    for i in range(5):
        await storage.append_transaction(111, f"0{i + 1}.08.2026", "Expense", "Cat", 10, "-")
...
async def test_tab_exists_cache_is_per_tab_name(mock_sheets_service):
    ...
    await storage.append_transaction(111, "01.08.2026", "Expense", "Cat", 10, "-")  # owner -> "Transactions"
    await storage.append_transaction(222, "01.08.2026", "Expense", "Cat", 10, "-")  # other user -> "Transactions_222"
...
async def test_append_transaction_self_heals_after_manual_tab_deletion(mock_sheets_service):
    _client._known_existing_tabs.add("Transactions")  # cache believes the tab still exists
    ...
    await storage.append_transaction(111, "01.08.2026", "Expense", "Кафе", 100, "-")
    ...
    assert "Transactions" in _client._known_existing_tabs  # cache repopulated after self-heal
...
async def test_append_transactions_batch_self_heals_after_manual_tab_deletion(mock_sheets_service):
    ...
    assert "Transactions" in _client._known_existing_tabs
...
async def test_set_budget_self_heals_after_manual_tab_deletion(mock_sheets_service):
    ...
    assert "Budgets" in _client._known_existing_tabs
