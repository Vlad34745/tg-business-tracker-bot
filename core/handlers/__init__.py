...
reports.py, find.py, and remind.py (to resume those flows from
free-text input), so those three must be imported before entries.py.
Every submodule is imported purely for its side effect of registering
handlers on `router` — nothing else needs to be re-exported except
what core/bot.py and tests import from this package.
"""
